...
// NVIDIA NIM VLM menerima gambar sebagai tag <img> base64 di dalam content.
// Model default: meta/llama-3.2-11b-vision-instruct (ringan, free endpoint).
async function visionChat(cfg, imageBase64, prompt, model, timeoutMs) {
    ...
    if (!key) throw new Error('NVIDIA API key kosong (Settings)');
    ...
    try {
        const res = await fetch(ENDPOINT, {
            ...
            body: JSON.stringify({
                ...
                messages: [{ role: 'user', content: `${prompt} <img src="data:image/jpeg;base64,${imageBase64}" />` }],
                max_tokens: 256, temperature: 0.2, stream: false,
            }),
            signal: ctrl.signal,
        });
        if (!res.ok) throw new Error(`NIM VLM ${res.status}: ${(await res.text()).slice(0, 200)}`);
        const j = await res.json();
        return j.choices[0].message.content;
