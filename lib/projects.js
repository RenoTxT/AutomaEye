...
// --- Model CRUD ---
exports.addModel = (root, projectName, { name, aiType, addons, classes }) => {
        ...
        .map(f => {
            ...
            if (fs.existsSync(lblPath)) {
                boxes = fs.readFileSync(lblPath, 'utf8').split(/\r?\n/).map(line => {
                    ...
                    const [cx, cy, w, h] = p.slice(1, 5).map(Number);
                    if ([cx, cy, w, h].some(v => Number.isNaN(v))) return null;
...
// baru split. Dipakai untuk memperbaiki dataset yang sudah terlanjur rusak.
exports.cleanRebuildDataset = (root, projectName, modelName, ratios) => {
    ...
    return { removedAug, fixedNames, ...split };
};
