...
exports.collect = async (cfg, project, imageBase64, runResult) => {
    const sl = cfg.self_learning || {};
    ...
    const jsonPath = path.join(d, stem + '.json');
    fs.writeFileSync(path.join(d, stem + '.jpg'), Buffer.from(imageBase64, 'base64'));
    const meta = {
    ...
    // AI triage opsional (VLM NVIDIA) — SARAN untuk ditinjau manusia, bukan label otomatis.
    let aiSuggestion = null;
    if (sl.ai_triage && (sl.api_key || (cfg.nvidia && cfg.nvidia.api_key))) {
        try {
            const nvidia = require('./nvidia');
            const m = (project.models || []).find(x => x.name === modelName);
            aiSuggestion = await nvidia.triageSample(cfg, imageBase64, m && m.classes);
            meta.aiSuggestion = aiSuggestion;
            fs.writeFileSync(jsonPath, JSON.stringify(meta, null, 2));   // simpan saran
        } catch (e) {
            aiSuggestion = { error: e.message };
        }
    }
