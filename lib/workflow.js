...
function evaluateAddons(m, detections, measureFromBox) {
    ...
    if (has('GD&T Measurement')) {
        ...
        if (!mmpp) {
            ...
            // Cek JUMLAH (n) per kelas: part harus punya sejumlah fitur tertentu.
            // Kalau belum lengkap → NG + incomplete (inspeksi menunggu sampai semua muncul).
            const countMsgs = [];
            Object.keys(perClass || {}).forEach(cls => {
                ...
                if (got < need) { incomplete = true; verdict = 'NG'; }
            });
            if (countMsgs.length) {
                checks.push({ addon: 'Jumlah', pass: !incomplete, detail: countMsgs.join(' · ') + (incomplete ? ' — belum lengkap' : ' ✓ lengkap') });
            }
        }
    }
