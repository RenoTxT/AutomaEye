            ...
            dets.forEach(d => {
                ...
                if (spec) {
                    TYPES.forEach(t => {
                        ...
                        parts.push(`${d.class_name} ${t.field}=${val.toFixed(2)}mm${ok ? '' : ` ✗(${nominal}±${tol})`}`);
                    });
                }
                // Kelas tanpa spesifikasi ber-nominal → tetap tampilkan UKURAN MENTAH (netral)
                // biar user langsung lihat dimensi & bisa menentukan nominal. ok:null = netral (tidak menilai).
                if (!labels.length) {
...
exports.execute = async (cfg, project, imageDataUrl, arduino, output, opts = {}) => {
    ...
    for (const step of steps) {
        ...
        try {
            await runStep(step, sr, { cfg, project, base64, arduino, result, conf });
        } catch (e) {
            sr.verdict = 'ERROR';
            sr.error = e.message;
        }
